...
public class TurretSubsystem extends SubsystemBase {
  ...
  private void HandleElevationLimits(Pose2d turretPose){
    ...
    if(((turretPose.getX() < Red.neutralZoneEnterX + buffer && turretPose.getX() > Red.neutralZoneEnterX - buffer) || (turretPose.getX() < Blue.neutralZoneEnterX + buffer && turretPose.getX() > Blue.neutralZoneEnterX - buffer)) && ((turretPose.getY() > FieldConstants.bottomTrenchStartY && turretPose.getY() < FieldConstants.bottomTrenchEndY) || (turretPose.getY() > FieldConstants.topTrenchStartY && turretPose.getY() < FieldConstants.topTrenchEndY))){
      canElevate = false;
    }else{
      canElevate = true;
    }
  ...
  public static void setElevation(double degrees){
    if(degrees == Double.NaN){
      degrees = TurretConstants.ElevationMaxAngle.getDegrees();
    }

    double returnDegrees = Math.max(Math.min(degrees, TurretConstants.ElevationMaxAngle.getDegrees()), TurretConstants.ElevationMinAngle.getDegrees());
  ...
  private static void configTurretCANCoder(){
    ...
    turretCANCoderConfig.MagnetSensor.MagnetOffset = 0.240478515625;
  ...
  private static void configTurret(){
    ...
    turretRotConfig.MotionMagic.MotionMagicAcceleration = 120;
    turretRotConfig.MotionMagic.MotionMagicCruiseVelocity = 60;
